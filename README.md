...
- [Модуль 1. Задача E](#модуль-1-задача-e)
- [Модуль 1. Задача F](#модуль-1-задача-f)
...
В данной задаче не требуется достичь хорошей асимптотической сложности. Достаточно просто сохранить все фигуры на поле - двумерном массиве. А после пройтись по всему полю и, встречая фигуры, во вложенном цикле отметить все допустимые ею ходы. После можно пройтись еще раз по всему полю и посчитать количество непокрытых клеток.

[Решение](Lesson_01/D/1D.cpp)

## [Модуль 1. Задача E](#модуль-1-задача-e)

<img src="TaskConditions/1E.png" alt="1E" width="500"/>

Базовое предположение в данной задаче - это на каждой итерации добавлять цифру из диапазона [0, 9]. Однако, если обратить внимания на ограничения, полученное число не влезет по крайней мере в стандартные численные типы языка C++. Поэтому можно хранить цифры в виде вектора. Но также стоит вопрос с проверкой делимости. Если новое число представить как new = (10 * prev + num), то можно думать над дальнейшим вычислением остатка, а можно сразу понять, что если какое-то число делится на k, то умноженное на 10 число (приписанный в конце ноль) будет делится на k. Соответственно достаточно найти следующую цифру, такую, чтобы новое число делилось на k, а затем приписать в конце просто (d - 1) нулей. Иначе сразу можно сказать, что такой цифры нет и вывести "-1".

[Решение](Lesson_01/E/1E.cpp)

## [Модуль 1. Задача F](#модуль-1-задача-f)

<img src="TaskConditions/1F.png" alt="1F" width="500"/>

Так как в условии гарантируется, что решение существует, то существует хотя бы одно нечетное число. Найдем первое нечетное число в наборе чисел и последнее в этой подпоследовательности (В пример - окно выделено вертикальными прямыми: {ч, ..., ч, |н, ..., н|, ч...}). Возможно три случая расположения этого окна (также стоит отметить, что окно можем быть единичной длины - одно нечетное число):

- {н, ..., н, ч, ...} - н х ... х н - даст нечетное число, ч х ... (любые числа) - даст четное число. А между ними поставим знак плюс н + ч = н
- {ч, ..., ч, н, ..., н} - ч + ... + ч - даст четное число, н х ... х н - нечетное. Аналогично, ч + н = н
- {ч, ..., ч, |н, ..., н|, ч...} - более общий случай, ч + ... + ч - четное, н х ... х н - нечетное, ч х ... - четное ч + н + ч = н

Таким образом, для решения задачи, нужно найти окно последовательности первых нечетных чисел (возможно, длины 1). Если есть числа слева, то просто суммируем их. А если есть хотя бы одно четное число справа, то прибавляем его, умноженного на все другие оставшиеся числа справа.

[Решение](Lesson_01/F/1F.cpp)